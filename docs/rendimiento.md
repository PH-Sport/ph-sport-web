...
- **Tres imágenes con margen, recomprimidas de verdad**: `contact-image.webp` 76 → 16 KB a 800w (no tiene variante móvil y se sirve la de 1600), `logo-ph-3d.webp` 427 → 199 KB en AVIF, `talents-hero.webp` 142 → 94 KB a q72 sin cambiar dimensiones. (El póster del hero también estaba en esta lista, 142 → 53 KB; quedó resuelto el 2026-09-22 con el vídeo nuevo: 76 KB en escritorio y un recorte de 34 KB para móvil.)
- ~~**Vídeo del hero a CRF 30**~~ — superado el 2026-09-22: el vídeo es otro (1080p, 17,7 s, HEVC + H.264) y el CRF de cada variante se eligió midiendo SSIM contra el master. Cifras en `DECISIONS.md`.
